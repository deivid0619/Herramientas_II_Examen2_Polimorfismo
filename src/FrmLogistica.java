...
public class FrmLogistica extends JFrame {
    ...
    public void quitarEnvio() {
        if (tblEnvios.getSelectedRow() >= 0) {
            ...
            JOptionPane.showMessageDialog(null, "SELECCIONE EL ENVIO A ELIMINAR");
        }
    ...
    private void mostrarEnvios() {
        ...
        for (Envio e : envios) {
            if (e != null) {
                datos[fila][0] = e.getCodigo();
                datos[fila][1] = e.getCliente();
                datos[fila][2] = df.format(e.getPeso());
                datos[fila][3] = df.format(e.getDistancia());
                datos[fila][4] = e instanceof EnvioTerrestre ? "Terrestre"
                        : e instanceof EnvioAereo ? "Aéreo" : "Marítimo";
                datos[fila][5] = df.format(e.calcularTarifa());
            }
            fila++;
        }
